...
export default function DashboardPage() {
  ...
  const dynamicHeroCard = useMemo((): DynamicHeroCard | null => {
    ...
    if (goingColdLeads.length >= 2) {
      ...
      return {
        type: 'going_cold',
        title: `⚠️ ${goingColdLeads.length} Leads Going Cold`,
  ...
  const getGreeting = () => {
    ...
    if (hour >= 6 && hour < 12) return { text: 'Good morning', emoji: '☀️', motivation: 'Ready to close some deals?' }
  ...
  const getActivityDisplay = (activity: LeadActivity) => {
    const types: Record<string, { icon: React.ReactNode; label: string; color: string }> = {
      'call': { icon: <Phone className="h-3 w-3" />, label: 'Phone Call', color: 'text-emerald-400' },
    ...
    return types[activity.activity_type] || { icon: <Activity className="h-3 w-3" />, label: activity.activity_type, color: 'text-slate-400' }
  }
    ...
    <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="fixed inset-0 pointer-events-none z-50 overflow-hidden">
          {[...Array(50)].map((_, i) => (
            ...
            >
              <div
                className={`w-3 h-3 ${
                  ['bg-emerald-400', 'bg-teal-400', 'bg-cyan-400', 'bg-amber-400', 'bg-pink-400', 'bg-purple-400'][
      ...
      {celebrationLead && showConfetti && (
        <div className="fixed inset-0 z-40 flex items-center justify-center p-4 pointer-events-none">
          <div className="bg-slate-900/95 backdrop-blur-xl border border-emerald-500/50 rounded-2xl p-8 shadow-2xl shadow-emerald-500/20 max-w-md animate-bounce-in pointer-events-auto">
            <div className="text-center">
              ...
              <div className="flex justify-center gap-3">
                <a
                  ...
                  className="flex items-center gap-2 bg-emerald-500 hover:bg-emerald-400 text-white px-5 py-3 rounded-xl font-semibold transition-all"
                >
                ...
                <button
                  ...
                  className="px-5 py-3 rounded-xl font-semibold text-slate-400 hover:text-white transition-colors"
                >
                  Later
                </button>
              </div>
      ...
      <div className="relative mb-6 flex items-start justify-between">
        <div>
          ...
          <p className="text-slate-500 text-xs mt-1">{greeting.motivation}</p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            disabled={isSeeding}
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white text-sm transition-all disabled:opacity-50 border border-slate-700"
          >
            {isSeeding ? (
              <>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">Toggle sections visibility</p>
                </div>
              </div>
              <button
                onClick={() => setShowCustomizeModal(false)}
            ...
            <div className="space-y-3">
              {[
                { key: 'hero', label: 'Priority Hero Card', icon: '🎯' },
                { key: 'kpis', label: 'KPI Cards', icon: '📊' },
                { key: 'actionQueue', label: 'Action Queue', icon: '⚡' },
                { key: 'activityFeed', label: 'Activity Feed', icon: '📋' },
                { key: 'insights', label: 'Daily Insights', icon: '💡' },
                { key: 'momentum', label: 'Weekly Momentum', icon: '📈' },
                { key: 'hotLeads', label: 'Hot Leads Section', icon: '🔥' },
              ].map(({ key, label, icon }) => (
                <button
                  key={key}
      ...
      {sectionVisibility.hero && dynamicHeroCard ? (
          ...
          <div className="relative p-6 sm:p-8">
            ...
            <div className="flex items-center gap-2 mb-6">
              ...
              <div className="flex-1">
                ...
                <p className="text-white/70 text-sm">{dynamicHeroCard.subtitle}</p>
              </div>
              
              {/* Urgency Badge */}
              <span className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium ${
                dynamicHeroCard.urgency === 'critical'
                ...
                {dynamicHeroCard.lead ? (
                  <>
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="relative flex-shrink-0">
                        <div className="w-18 h-18 sm:w-22 sm:h-22 rounded-2xl bg-white/10 backdrop-blur flex items-center justify-center text-3xl sm:text-4xl font-bold text-white border-2 border-white/20">
                        ...
                        <div className="absolute -bottom-2 -right-2 w-11 h-11 sm:w-13 sm:h-13">
                          ...
                          <div className="absolute inset-0 flex items-center justify-center">
                            <span className="text-sm font-bold text-white">{dynamicHeroCard.lead.intent_score}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {/* Lead Details */}
                        <div className="flex flex-wrap items-center gap-3 mt-2 text-white/70 text-sm">
                    ...
                    <div className="bg-white/10 backdrop-blur rounded-xl p-4 border border-white/10">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <p className="text-white/60 text-xs uppercase tracking-wide mb-1">Key Insight</p>
                          <p className="text-white font-medium">{dynamicHeroCard.insight}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-3">
                      <a
                        ...
                        className="flex items-center gap-2 bg-white text-slate-900 hover:bg-white/90 px-5 py-3 rounded-xl font-semibold transition-all hover:scale-105 shadow-lg"
                      >
                        <Phone className="h-5 w-5" />
                        Call Now
                      </a>
                      <a
                        ...
                        className="flex items-center gap-2 bg-white/20 hover:bg-white/30 text-white px-5 py-3 rounded-xl font-semibold transition-all border border-white/20"
                      >
                      ...
                      <Link
                        ...
                        className="flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white px-5 py-3 rounded-xl font-semibold transition-all border border-white/10"
                      >
                  ...
                  <div className="space-y-5">
                    <div className="bg-white/10 backdrop-blur rounded-xl p-6 border border-white/10">
                      <p className="text-white text-lg font-medium mb-3">{dynamicHeroCard.insight}</p>
                      <p className="text-white/60 text-sm">{dynamicHeroCard.action}</p>
                    </div>
                    
                    <Link
                      href="/dashboard/leads"
                      className="inline-flex items-center gap-2 bg-white text-slate-900 hover:bg-white/90 px-6 py-3 rounded-xl font-semibold transition-all hover:scale-105 shadow-lg"
                    >
                      ...
                      {dynamicHeroCard.lead.recentActivities.slice(0, 3).map((activity) => {
                        ...
                        return (
                          <div key={activity.id} className="flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-white/90 text-sm font-medium">{display.label}</p>
        ...
        <div className="relative mb-8 rounded-2xl overflow-hidden">
          ...
          <div className="relative p-8 sm:p-12 text-center">
            ...
            <h2 className="text-xl font-semibold text-white mb-2">Welcome to Your Command Center!</h2>
            <p className="text-slate-400 mb-6 max-w-md mx-auto">
              Add your first lead to unlock AI-powered insights, priority recommendations, and smart action suggestions.
            </p>
            <Link
              href="/dashboard/leads"
              className="inline-flex items-center gap-2 bg-emerald-500 hover:bg-emerald-400 text-white px-6 py-3 rounded-xl font-semibold transition-all"
            >
              Add Your First Lead
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <Link
          ...
          className="group relative bg-gradient-to-br from-emerald-500/10 via-teal-500/5 to-slate-900/50 backdrop-blur-xl border border-emerald-500/20 rounded-2xl p-5 hover:border-emerald-500/40 hover:shadow-lg hover:shadow-emerald-500/10 hover:-translate-y-1 transition-all duration-300"
        >
          ...
          <div className="relative">
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-emerald-500/30 to-teal-500/20 flex items-center justify-center">
            ...
            <div className="mt-3 mb-2">
              ...
              <p className="text-xs text-slate-500 mt-1">{kpiMetrics.hotPipeline.targetProgress}% of monthly target</p>
            </div>
        ...
        {/* CARD 2: Next 24 Hours */}
        <div className="group relative bg-gradient-to-br from-teal-500/10 via-cyan-500/5 to-slate-900/50 backdrop-blur-xl border border-teal-500/20 rounded-2xl p-5 hover:border-teal-500/40 hover:shadow-lg hover:shadow-teal-500/10 hover:-translate-y-1 transition-all duration-300 cursor-pointer">
          ...
          <div className="relative">
            {/* Header */}
            <div className="flex items-center justify-between mb-3">
              <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-teal-500/30 to-cyan-500/20 flex items-center justify-center relative">
                <Clock className="h-5 w-5 text-teal-400" />
                {kpiMetrics.followUp.count > 0 && (
                  <span className="absolute -top-1 -right-1 w-4 h-4 bg-amber-500 rounded-full flex items-center justify-center text-[10px] font-bold text-white animate-pulse">
                    !
                  </span>
                )}
              </div>
            ...
            {kpiMetrics.followUp.leads.length > 0 && (
              <div className="flex items-center gap-1 mb-3">
                {kpiMetrics.followUp.leads.slice(0, 4).map((lead, i) => (
                  <div
                    key={lead.id}
                    className="w-7 h-7 rounded-full bg-gradient-to-br from-teal-500/30 to-cyan-500/20 border-2 border-slate-800 flex items-center justify-center text-xs font-medium text-white -ml-1 first:ml-0"
                    title={lead.name}
                  >
                    {lead.name.charAt(0)}
                  </div>
        ...
        <Link
          ...
          className="group relative bg-gradient-to-br from-cyan-500/10 via-teal-500/5 to-slate-900/50 backdrop-blur-xl border border-cyan-500/20 rounded-2xl p-5 hover:border-cyan-500/40 hover:shadow-lg hover:shadow-cyan-500/10 hover:-translate-y-1 transition-all duration-300"
        >
          ...
          <div className="relative">
            {/* Header */}
            <div className="flex items-center justify-between mb-3">
              <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-cyan-500/30 to-teal-500/20 flex items-center justify-center">
            ...
            <div className="mb-2">
              ...
              <p className="text-xs text-slate-400">Avg time to hot status</p>
            </div>
                ...
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <div 
                    className={`w-full rounded-sm transition-all ${
                      i === kpiMetrics.velocity.trend.length - 1 
                        ? 'bg-cyan-500' 
                        : 'bg-cyan-500/30'
                    }`}
                    style={{ 
                  ...
                  />
                  <span className="text-[9px] text-slate-500">{point.week}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* CARD 4: Today's Momentum */}
        <div className="group relative bg-gradient-to-br from-emerald-500/10 via-cyan-500/5 to-slate-900/50 backdrop-blur-xl border border-emerald-500/20 rounded-2xl p-5 hover:border-emerald-500/40 hover:shadow-lg hover:shadow-emerald-500/10 hover:-translate-y-1 transition-all duration-300 cursor-pointer">
          <div className="absolute inset-0 bg-gradient-to-br from-emerald-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity rounded-2xl" />
          
          <div className="relative">
            {/* Header */}
            <div className="flex items-center justify-between mb-3">
              <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-emerald-500/30 to-cyan-500/20 flex items-center justify-center">
            ...
            <div className="flex flex-wrap gap-2 mb-3 text-xs">
              {kpiMetrics.momentum.breakdown.calls > 0 && (
                <span className="text-slate-400">📞 {kpiMetrics.momentum.breakdown.calls}</span>
              )}
              {kpiMetrics.momentum.breakdown.whatsapp > 0 && (
                <span className="text-slate-400">💬 {kpiMetrics.momentum.breakdown.whatsapp}</span>
              )}
              {kpiMetrics.momentum.breakdown.visits > 0 && (
                <span className="text-slate-400">🏠 {kpiMetrics.momentum.breakdown.visits}</span>
              )}
              {kpiMetrics.momentum.breakdown.emails > 0 && (
                <span className="text-slate-400">✉️ {kpiMetrics.momentum.breakdown.emails}</span>
              )}
            </div>
            ...
            <div className="flex items-end gap-1 h-6">
              {kpiMetrics.momentum.weekTrend.map((day, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-0.5">
                  <div 
                    className={`w-full rounded-sm transition-all ${
                      i === kpiMetrics.momentum.weekTrend.length - 1 
                        ? 'bg-emerald-500' 
                        : 'bg-emerald-500/30'
                    }`}
                    style={{ 
                  ...
                  />
                  <span className="text-[8px] text-slate-500">{day.day}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 mb-8">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-lg font-semibold text-white flex items-center gap-2">
            ...
            Smart Action Queue
            <span className="text-sm font-normal text-slate-400">— Do These Next</span>
          </h2>
        ...
        {actionQueue.length > 0 ? (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-700/50">
                  <th className="text-left text-xs font-medium text-slate-500 uppercase tracking-wider pb-3 pl-2">#</th>
                  <th className="text-left text-xs font-medium text-slate-500 uppercase tracking-wider pb-3">Lead</th>
                  <th className="text-left text-xs font-medium text-slate-500 uppercase tracking-wider pb-3 hidden sm:table-cell">AI Recommendation</th>
                  <th className="text-left text-xs font-medium text-slate-500 uppercase tracking-wider pb-3 hidden md:table-cell">Urgency</th>
                  <th className="text-right text-xs font-medium text-slate-500 uppercase tracking-wider pb-3 pr-2">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/50">
                {actionQueue.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="py-3">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-emerald-500/20 to-teal-500/20 flex items-center justify-center text-sm font-bold text-white flex-shrink-0">
                          {item.lead.name.charAt(0)}
                        </div>
                        <div className="min-w-0">
                          <p className="font-medium text-white truncate max-w-[120px] sm:max-w-[180px]">
                            {item.lead.name}
                          </p>
                          ...
                          <p className="sm:hidden text-xs text-emerald-400/80 mt-1 flex items-center gap-1">
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-white mb-2">🎉 Amazing! You&apos;re all caught up.</h3>
            <p className="text-slate-400 text-sm max-w-md mx-auto mb-6">
              No pending actions right now. Add more leads or review your closed deals.
            </p>
            <Link
              href="/dashboard/leads"
              className="inline-flex items-center gap-2 bg-emerald-500 hover:bg-emerald-400 text-white px-5 py-2.5 rounded-xl font-medium transition-all text-sm"
            >
        ...
        {/* Legend */}
        {actionQueue.length > 0 && (
          <div className="flex flex-wrap items-center justify-center gap-4 mt-6 pt-4 border-t border-slate-800">
      ...
      <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 mb-8">
        ...
        {activities.length > 0 ? (
          <div className="max-h-[400px] overflow-y-auto custom-scrollbar space-y-2 pr-2">
            {activities.slice(0, 30).map((activity) => {
              ...
              return (
                <div
                  key={activity.id}
                  className={`flex items-start gap-3 p-3 rounded-xl border-l-2 transition-colors hover:bg-slate-800/50 ${outcomeStyle}`}
                >
                  {/* Activity Icon */}
                  <div className={`w-8 h-8 rounded-lg bg-slate-800 flex items-center justify-center flex-shrink-0 ${activityInfo.color}`}>
                    {activityInfo.icon}
                  </div>
          ...
          <div className="text-center py-12">
            ...
            <p className="text-slate-500 text-xs mt-1">Log your first activity on a lead to see it here</p>
          </div>
        )}
        ...
        <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {dailyInsights.map((insight) => (
              <div
                key={insight.id}
                className={`relative overflow-hidden rounded-xl p-5 border transition-all hover:shadow-lg ${
                  insight.type === 'success'
                    ? 'bg-gradient-to-br from-emerald-500/10 via-emerald-500/5 to-transparent border-emerald-500/20 hover:border-emerald-500/40'
              ...
              >
                ...
                <div className="relative">
                  ...
                  <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-700/50">
                    <p className="text-xs text-slate-500 flex-1 mr-3">
                      💡 {insight.action}
                    </p>
                    <Link
      ...
      <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 mb-8">
        ...
        <div className="grid grid-cols-7 gap-2 sm:gap-4 mb-6">
          {weeklyMomentum.days.map((day, index) => {
            ...
            return (
              <div
                key={index}
                className={`group relative flex flex-col items-center p-2 sm:p-3 rounded-xl transition-all cursor-pointer ${
                  isToday 
              ...
              >
                ...
                {day.newLeads > 0 && (
                  <div 
                    className="absolute top-14 left-1/2 -translate-x-1/2 rounded-full bg-emerald-500 border-2 border-slate-900 flex items-center justify-center"
                    style={{ 
                      width: `${leadSize}px`, 
                      height: `${leadSize}px`,
                    }}
                  >
                    {day.newLeads > 1 && (
                      <span className="text-[8px] font-bold text-white">{day.newLeads}</span>
                    )}
                  </div>
                )}
                ...
                {day.hotConversions > 0 && (
                  <div className="absolute top-8 right-0 sm:right-1">
                    ...
                    {day.hotConversions > 1 && (
                      <span className="absolute -bottom-1 -right-1 text-[8px] font-bold text-amber-400 bg-slate-900 rounded-full px-1">
                        {day.hotConversions}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 shadow-xl min-w-[140px]">
                    ...
                    <div className="space-y-1 text-xs">
                      <div className="flex justify-between">
                        ...
                        <span className="text-emerald-400 font-medium">{day.newLeads}</span>
                      </div>
                      <div className="flex justify-between">
        ...
        {/* Legend */}
        <div className="flex flex-wrap items-center justify-center gap-4 mb-4 pb-4 border-b border-slate-800">
          <div className="flex items-center gap-1.5 text-xs text-slate-500">
        ...
        <div className="bg-slate-800/30 rounded-xl p-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex flex-wrap items-center gap-4 sm:gap-6">
              <div className="text-center">
                <p className="text-2xl font-bold text-emerald-400">{weeklyMomentum.thisWeek.leads}</p>
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-slate-500">Became Hot</p>
              </div>
            </div>
            ...
            {leads
              ...
              .map(lead => (
                <Link
                  key={lead.id}
                  href={`/dashboard/leads/${lead.id}`}
                  className="bg-slate-800/50 hover:bg-slate-800 rounded-xl p-4 transition-all border border-slate-700/50 hover:border-emerald-500/30 group"
                >
                  <div className="flex items-start gap-3">
                    <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500/20 to-teal-500/20 flex items-center justify-center text-lg font-bold text-white">
                      {lead.name.charAt(0)}
                    </div>
                    <div className="flex-1 min-w-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Link
          href="/dashboard/leads"
          className="bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-xl p-5 hover:bg-slate-800/50 hover:border-emerald-500/30 transition-all group"
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="font-semibold text-white group-hover:text-emerald-400 transition-colors">Manage Leads</p>
              <p className="text-sm text-slate-400">View & add leads</p>
            </div>
          </div>
        </Link>
        ...
        <Link
          href="/dashboard/analytics"
          className="bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-xl p-5 hover:bg-slate-800/50 hover:border-teal-500/30 transition-all group"
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="font-semibold text-white group-hover:text-teal-400 transition-colors">Analytics</p>
              <p className="text-sm text-slate-400">Deep insights</p>
            </div>
          </div>
        </Link>
      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col-reverse items-end gap-3">
        ...
        <div className={`flex flex-col-reverse gap-3 transition-all duration-300 ${
          ...
          >
            <span className="bg-slate-800 text-slate-300 text-sm px-3 py-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
              Settings
            </span>
          ...
          >
            ...
            <div className="w-12 h-12 rounded-full bg-emerald-500 hover:bg-emerald-400 flex items-center justify-center shadow-lg shadow-emerald-500/30 transition-all hover:scale-110">
        ...
        <button
          ...
          className={`w-14 h-14 rounded-full flex items-center justify-center shadow-xl transition-all duration-300 ${
            fabOpen
              ...
              : 'bg-gradient-to-br from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 hover:scale-110 shadow-emerald-500/40'
          }`}
        >
          ...
          <div className="relative bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-md shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-white">Quick Log Activity</h3>
                  <p className="text-xs text-slate-500">Log in seconds</p>
                </div>
              </div>
              <button
                onClick={() => setQuickLogOpen(false)}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                >
                  <option value="">Choose a lead...</option>
                  {leads.map(lead => (
                    <option key={lead.id} value={lead.id}>
                      {lead.name} {lead.intent_score >= 70 ? '🔥' : lead.intent_score >= 50 ? '⭐' : ''}
                    </option>
                  ))}
                </select>
              </div>
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`flex flex-col items-center gap-1 p-3 rounded-xl border transition-all ${
                        quickLogForm.activityType === type
              ...
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">
                  Quick Note <span className="text-slate-500">(optional)</span>
                </label>
                <input
                  type="text"
                  ...
                  className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                />
              </div>
            </div>
            ...
            <div className="flex items-center justify-between mt-6 pt-4 border-t border-slate-800">
              ...
              <button
                ...
                className="flex items-center gap-2 bg-emerald-500 hover:bg-emerald-400 disabled:bg-slate-700 disabled:text-slate-500 text-white px-5 py-2.5 rounded-xl font-medium transition-all disabled:cursor-not-allowed"
              >
                {quickLogLoading ? (
                  <>
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">Opens WhatsApp</p>
                </div>
              </div>
              <button
                onClick={() => setQuickMessageOpen(false)}
            ...
            <div className="max-h-[300px] overflow-y-auto custom-scrollbar space-y-2">
              {leads.filter(l => l.phone).slice(0, 10).map(lead => (
                <button
                  ...
                  className="w-full flex items-center gap-3 p-3 rounded-xl bg-slate-800/50 hover:bg-slate-800 border border-slate-700/50 hover:border-teal-500/30 transition-all text-left"
                >
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-teal-500/20 to-emerald-500/20 flex items-center justify-center text-sm font-bold text-white flex-shrink-0">
                    {lead.name.charAt(0)}
                  </div>
                  <div className="flex-1 min-w-0">
